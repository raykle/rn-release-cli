...
function uploadSourceMap(options) {
  return new Promise((resolve, reject) => {
    ...
    const loading = `${Constants.RNReleaseCLI} bugsnag source map uploading...`
    const spinner = ora(loading);
    spinner.start();

    upload(bugsnagOptions, (err, message) => {
      if (err) {
        spinner.fail(`${failPrompt} source map upload failed: ${err.message}.`)
        reject(err.message)
        return
      }
      spinner.succeed(`${succeedPrompt} source map upload succeed.`)
      resolve()
    })
  })
}
