...
function rnBundleAsync(options) {
  return new Promise((resolve, reject) => {
    ...
    const spinner = ora(`${Constants.RNReleaseCLI} ${Constants.ReactNativePromptStr} bundling...`)
    spinner.start()
    ...
    bundleProcess.on('close', (code) => {
      if (code === 0) {
        spinner.succeed(`${succeedPrompt} bundle succeed.`);
        resolve()
      } else {
        spinner.fail(`${failPrompt} bundle failed: ${chalk.red(code)}.`);
        process.exit(1)
      }
    })
